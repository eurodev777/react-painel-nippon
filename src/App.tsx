...
import { Loader2, CheckCircle2, ShieldAlert, Plus, Users, Search, AlertCircle, X, ShieldCheck } from 'lucide-react';
...
export default function App() {
  ...
  const carregarDados = async (retry = false) => {
    ...
    try {
      ...
      console.warn('API falhou ou bloqueio de CORS. Usando fallback offline do LocalStorage.');
      ...
      if (localBackup) {
        ...
        setDados({ categorias: DEFAULT_CATEGORIES });
        localStorage.setItem('nippon_atletas_data', JSON.stringify({ categorias: DEFAULT_CATEGORIES }));
      }
  ...
  const resetParaDadosPadrao = () => {
    if (window.confirm('Tem certeza que deseja redefinir todas as tabelas para os dados esportivos padrões? Suas alterações locais serão perdidas.')) {
      setDados({ categorias: DEFAULT_CATEGORIES });
      localStorage.setItem('nippon_atletas_data', JSON.stringify({ categorias: DEFAULT_CATEGORIES }));
      setCategoriaAtual(0);
      setIsOfflineMode(true);
  ...
  const salvarAlteracaoNoServidor = async (
    ...
    if (ehNovoRegistro && (tipo === 'equipe' || tipo === 'atleta')) {
      if (!parentId || String(parentId) === 'undefined' || String(parentId) === 'null' || String(parentId).startsWith('novo-')) {
        console.error(`[BLOQUEIO] Objeto pai da ${tipo} não possui um ID válido no banco de dados ainda.`, parentId);
        alert(
        ...
        if (ehNovoRegistro) {
          ...
          if (tipo === 'categoria') {
            ...
            if (typeof atl === 'object' && atl !== null) {
              ...
              novosDados.categorias[categoriaAtual].equipes[eqIdx].atletas[atlIdx] = {
                id: id,
                nome: novoValor
              };
            }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Servidor retornou erro status ${response.status}. Detalhes: ${textoResposta}`);
      }
      ...
      alert(`[Servidor de Banco] Não foi possível salvar: ${error.message}.\nO item foi mantido temporariamente no seu navegador.`);
    }
  };
  ...
  const adicionarCategoriaLocal = async (titulo: string) => {
    ...
    if (!isOfflineMode) {
      try {
        ...
        const response = await fetch('https://sothink.com.br/apinippon/api/v2/nippon/inserir', {
          method: 'POST',
          body: formData,
        });
        ...
        if (resJson.sucesso && resJson.id_inserido) {
          ...
          setDados(dadosComId);
          localStorage.setItem('nippon_atletas_data', JSON.stringify(dadosComId));
        }
        setSavingStatus('saved');
        setTimeout(() => setSavingStatus('idle'), 2000);
      } catch (err) {
        console.warn('Erro ao inserir categoria remotamente. Mantido localmente.', err);
  ...
  const removerAtletaLocal = (eqIdx: number, atlIdx: number) => {
    if (!dados) return;
    const novosDados = { ...dados };
    novosDados.categorias[categoriaAtual].equipes[eqIdx].atletas.splice(atlIdx, 1);
    setDados(novosDados);
    localStorage.setItem('nippon_atletas_data', JSON.stringify(novosDados));
    showToastNotification();
  };
  ...
  const getFilteredSearchResults = () => {
    ...
    const results: { categoryTitle: string; teamName: string; athleteName?: string; type: 'team' | 'athlete' }[] = [];
  ...
  if (loading) {
    return (
      <div className="bg-stone-50 min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        />
        <h3 className="text-stone-800 font-bold text-lg font-display">NIPPON COUNTRY CLUB</h3>
        <p className="text-stone-400 text-sm mt-1">Carregando painel de gerenciamento oficial de atletas...</p>
      </div>
    );
  }
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-12 px-4 max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <p className="text-stone-500 text-sm md:text-base leading-relaxed mb-8">
              Gerencie a relação oficial de equipes e atletas federados em todas as categorias desportivas de forma integrada e segura.
            </p>

            {/* Quick stats grid */}
            <div className="grid grid-cols-3 gap-3 md:gap-4 mb-8 bg-stone-50 p-4 rounded-2xl border border-stone-200/50">
              <div className="text-center">
                <p className="text-xl md:text-2xl font-black text-stone-900 leading-none">{totalCats}</p>
                <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mt-1.5 font-mono">Categorias</p>
              </div>
              <div className="text-center border-x border-stone-200">
                <p className="text-xl md:text-2xl font-black text-stone-900 leading-none">{totalEqs}</p>
                <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mt-1.5 font-mono">Equipes</p>
              </div>
              <div className="text-center">
                <p className="text-xl md:text-2xl font-black text-stone-900 leading-none">{totalAtls}</p>
                <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mt-1.5 font-mono">Atletas</p>
              </div>
            </div>
        ...
        <>
          ...
          <div className="fixed bottom-6 right-6 bg-white/95 backdrop-blur-md shadow-2xl border border-stone-200/80 rounded-2xl p-4 z-50 flex items-center gap-3 text-sm max-w-sm animate-fade-in">
            {savingStatus === 'idle' && (
              <>
                ...
                <span className="text-stone-500 font-medium text-xs">As alterações são salvas ao desfocar dos campos.</span>
              </>
            )}
            {savingStatus === 'saving' && (
              <>
                <Loader2 className="w-4 h-4 animate-spin text-red-600" />
                <span className="text-red-600 font-bold text-xs">Atualizando banco...</span>
              </>
            )}
            {savingStatus === 'saved' && (
              <>
                <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                <span className="text-emerald-600 font-bold text-xs">Modificações salvas com sucesso!</span>
              </>
            )}
            {savingStatus === 'error' && (
              <>
                <AlertCircle className="w-4 h-4 text-red-500" />
                <span className="text-red-500 font-bold text-xs">Falha de gravação no banco.</span>
              </>
            )}
          </div>
          ...
          {!searchTerm && (
            <CategoryTabs
              ...
              onRemoverCategoriaLocal={removerCategoriaLocal}
            />
          )}
          ...
          <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {searchTerm ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-stone-400">Total de {searchResults.length} correspondências encontradas</p>
                  </div>
                  <button
                ...
                {searchResults.length > 0 ? (
                      ...
                      >
                        <div>
                          <span className="inline-block text-[9px] font-bold px-2 py-0.5 rounded-sm bg-stone-100 text-stone-500 uppercase tracking-widest font-mono mb-2">
                            {res.categoryTitle.replace('RELAÇÃO DOS ATLETAS - ', '')}
                          </span>
                          <h4 className="font-bold text-stone-800 text-sm uppercase">{res.teamName}</h4>
                          {res.athleteName && (
                            <p className="text-xs text-stone-500 mt-1 flex items-center gap-1.5 bg-stone-50 p-2 rounded-lg border border-stone-100">
                              ...
                              <span>Atleta: <strong className="text-stone-700">{res.athleteName}</strong></span>
                            </p>
                          )}
                        </div>
                        <button
                          onClick={() => {
                            const catIdx = dados?.categorias.findIndex(c => c.titulo === res.categoryTitle);
                  ...
                  <div className="bg-white rounded-2xl border border-stone-200/80 p-12 text-center max-w-md mx-auto">
                    ...
                    <h4 className="font-bold text-stone-800">Nenhum resultado encontrado</h4>
                    <p className="text-xs text-stone-400 mt-1">Refine seus termos de busca ou tente pesquisar por capitães escrevendo "(C)"</p>
                  </div>
                )}
              </div>
            ) : (
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs">
                    <div className="flex-1">
                      <input
                        ...
                        onBlur={() => salvarAlteracaoNoServidor('categoria', obterId(activeCategoryObj), activeCategoryObj.titulo)}
                      ...
                      <p className="text-[11px] text-stone-400 mt-1.5 px-2 font-medium">
                        Edite o título acima. As equipes serão salvas sob esta categoria ({activeCategoryObj.equipes?.length || 0} equipes inscritas).
                      </p>
                    </div>
                ...
                {activeCategoryObj && activeCategoryObj.equipes && activeCategoryObj.equipes.length > 0 ? (
                    ...
                    <AnimatePresence mode="popLayout">
                      {activeCategoryObj.equipes.map((equipe, eqIdx) => (
                        <TeamCard
                          ...
                          onRemoverEquipeLocal={removerEquipeLocal}
                          onRemoverAtletaLocal={removerAtletaLocal}
                        />
                      ))}
                    </AnimatePresence>
                  </div>
                  ...
                  <div className="bg-white rounded-3xl border border-stone-200/80 p-16 text-center max-w-xl mx-auto">
                    ...
                    <h4 className="text-lg font-bold text-stone-900 font-display">Sem Equipes Cadastradas</h4>
                    <p className="text-stone-400 text-sm mt-2 max-w-sm mx-auto">
                      Não há nenhuma equipe inscrita nesta categoria esportiva ainda. Comece clicando no botão abaixo para registrar a primeira.
                    </p>
                    <button
                      onClick={adicionarEquipeLocal}
